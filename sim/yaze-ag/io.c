...
// Default to STDOUT/STDIN
static struct Device conindev={{0},{0},NULL,"CON",0,0,0};
static struct Device *conin=&conindev;
static struct Device conoutdev={{0},{0},NULL,"CON",1,0,1};
...
{
  if (r->name != NULL && r->name[0]>='0' && r->name[0]<='9') /* open socket */ /*{{{*/
  {
  ...
  {
    // If name is not filled in, the assumption is that fd is already something meaningful (such as STDOUT)
    if (r->name != NULL) {
...
int bios_init(const char *initfile)
{
  ...
  /*}}}*/
  if (strcmp_safe(conindev.name,conoutdev.name)==0) conindev.wr=1;
  devopen(conin);
  atexit(bios_exit);
  if (strcmp_safe(conindev.name,conoutdev.name)==0) conout=conin; else devopen(conout);
  devopen(lst);
  if (strcmp_safe(pundev.name,rdrdev.name)==0) rdrdev.wr=1;
  devopen(rdr);
  if (strcmp_safe(pundev.name,rdrdev.name)==0) pun=rdr; else devopen(pun);
  /* open first drive */ /*{{{*/
